...
// Two auth modes:
//  - OIDC (default for stores connected since 2026): SDK uses BLOB_STORE_ID +
//    the auto-injected VERCEL_OIDC_TOKEN. No static token, nothing to pass.
//  - Static token: BLOB_READ_WRITE_TOKEN (or <PREFIX>_BLOB_READ_WRITE_TOKEN).
function resolveToken(){
  if (process.env.BLOB_READ_WRITE_TOKEN) return process.env.BLOB_READ_WRITE_TOKEN;
  for (const k in process.env){
    if (k.endsWith('BLOB_READ_WRITE_TOKEN') && process.env[k]) return process.env[k];
  }
  return null;
}
const TOKEN = resolveToken();
// Real OIDC needs the runtime token, not just BLOB_STORE_ID — otherwise the SDK
// has no credentials and calls throw. Gating on it keeps "unconfigured" a clean 503.
const HAS_OIDC = !!process.env.VERCEL_OIDC_TOKEN;
// Only hand the SDK a token when we truly have one; otherwise it falls back to OIDC.
const OPT = TOKEN ? { token: TOKEN } : {};
export function haveBlob(){ return !!(TOKEN || HAS_OIDC); }
...
// --- blob read / write (PRIVATE store) ---
// Private blobs aren't fetchable by URL — read them with get() through the SDK.
// useCache:false so a read right after a write never returns the stale CDN copy.
export async function readJson(pathname){
  const r = await get(pathname, { access: 'private', useCache: false, ...OPT });
...
export async function delPrefix(prefix){
  const { blobs } = await list({ prefix, ...OPT });
  if (blobs.length) await del(blobs.map(b => b.url), OPT);
}
